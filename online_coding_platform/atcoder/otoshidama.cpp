...
The commonly used bills in Japan are 10000-yen, 5000-yen and
1000-yen bills. Below, the word "bill" refers to only these.

According to Aohashi, he received an otoshidama (New Year money gift) envelope from his grandfather that contained N bills for a total of
...
int main()
{
     ...
     for(int i=0;i<=n;i++)
     {
          for(int j=0;j<=n;j++)
          {
               long long int res = y - 10000*i - 5000*j;
               if(res >= 0 && res%1000 == 0)
               {
                    int k = res/1000;
                    if(i+j+k == n)
